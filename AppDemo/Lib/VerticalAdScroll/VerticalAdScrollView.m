...
@interface VerticalAdScrollView ()
...
/** 标记是否正在轮播 */
@property(nonatomic, assign) BOOL isInICarousel;

/** 标记该停止定时器 */
@property(nonatomic, assign) BOOL shouldDismissTimer;

@end

@implementation VerticalAdScrollView
...
//重写 set 方法
- (void)setIsInICarousel:(BOOL)isInICarousel {
    _isInICarousel = isInICarousel;
    
    if (!isInICarousel) { //停止滚动时, 检查是否该停止定时器
        if (self.shouldDismissTimer) {
            [self performSelector:@selector(stopTimer) withObject:self afterDelay:0.2];
            self.shouldDismissTimer = NO;
        }
...
- (void)closeTimer {
    if (self.isInICarousel) { //如果正在轮播,  则设置应该关闭轮播的标记,  返回
        self.shouldDismissTimer = YES;
        return;
    }
    [self stopTimer];
}
...
- (void)refreshVisibleCellAppearance{
    ...
    for (NSInteger i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
        VerticalView *cell = [_cells objectAtIndex:i];
...
//            
//            CGFloat alpha = 0.3 - (delta / _pageSize.width);
//            if (alpha < 0) {
//                alpha = 0.0;
//            }
//            cell.customAlpha = alpha;
//            
...
- (void)setPageAtIndex:(NSInteger)pageIndex{
    ...
    if ((NSObject *)cell == [NSNull null]) {
        ...
        cell = [_dataSource flowView:self cellForVerticalPageAtIndex:pageIndex % self.orginPageCount];
...
- (void)reloadData{
    ...
    if (_needsReload) {
        ...
        //重置pageCount
        if (_dataSource && [_dataSource respondsToSelector:@selector(numberOfVerticalPagesInFlowView:)]) {
            
            //原始页数
            self.orginPageCount = [_dataSource numberOfVerticalPagesInFlowView:self];
            
            //总页数
            if (self.isCarousel) {
                _pageCount = self.orginPageCount == 1 ? 1: [_dataSource numberOfVerticalPagesInFlowView:self] * 3;
            }else {
                _pageCount = self.orginPageCount == 1 ? 1: [_dataSource numberOfVerticalPagesInFlowView:self];
            }
        ...
        //重置pageWidth
        if (_delegate && [_delegate respondsToSelector:@selector(sizeForVerticalPageInFlowView:)]) {
            _pageSize = [_delegate sizeForVerticalPageInFlowView:self];
        }
...
- (UIView *)dequeueReusableCell{
    VerticalView *cell = [_reusableCells lastObject];
    if (cell){
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if ([_delegate respondsToSelector:@selector(didScrollToVerticalPage:inFlowView:)] && _currentPageIndex != pageIndex) {
        [_delegate didScrollToVerticalPage:pageIndex inFlowView:self];
    }
...
- (void)singleCellTapAction:(UIGestureRecognizer *)gesture {
    ...
    if ([self.delegate respondsToSelector:@selector(didSelectVerticalCell:withSubViewIndex:)]) {
        
        [self.delegate didSelectVerticalCell:gesture.view withSubViewIndex:gesture.view.tag];
